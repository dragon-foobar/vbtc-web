...
export const ReactBtcPayButton = ({
  ...
  currencyOptions = ["SATS", "AUD", "BTC"], // Default
  ...
  const handleBlur = () => {
    ...
    setPrice((prevPrice) => Number(prevPrice).toFixed(8).toString());
  };
  ...
  const handleSliderChange = (e: ChangeEvent) => {
    if (!!e.target) {
      ...
      const scrubbedValue = parseInt(value, 10);
      if (
        !isNaN(scrubbedValue) &&
        scrubbedValue >= sliderMin &&
        scrubbedValue <= sliderMax
      ) {
        setPrice(scrubbedValue.toString());
      }
    }
  };
  ...
  const handlePriceChange = (e: ChangeEvent) => {
    ...
    newPrice =
      currency === "BTC" ? parseFloat(rawValue) : parseInt(rawValue, 10);
  ...
  const handlePlusMinus = (
    ...
    const newPrice =
      type === "-"
        ? Math.max(parsedPrice - step, min)
        : Math.min(parsedPrice + step, max);
    setPrice(newPrice.toString());
  };
  ...
  return (
    <form method="POST" onSubmit={handleSubmit}>
      ...
      >
        ...
        {imageShow && (
          <img src={`${btcpayUrl}/img/paybutton/logo.svg`} alt="BTCPay Logo" />
        )}
      </button>
    </form>
  );
